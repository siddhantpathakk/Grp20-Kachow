...
public class BluetoothServices {
    ...
    private static final String appName = "Group_20";
    ...
    public void stopClientThread(BluetoothDevice device, UUID uuid)
    {
        Log.d(TAG, "stopClient: Started.");

        Toast.makeText(mContext,"Disconnecting from device",Toast.LENGTH_SHORT).show();
        mConnectThread.cancel();
    ...
    private class ConnectedThread extends Thread{
        ...
        public void cancel(){
            ...
            try{
                ...
                connectionStatus.putExtra("Status", "disconnected");
                connectionStatus.putExtra("Device", mDevice);
                connectionStatus.putExtra("IsDisconnectButtonClicked", true);
                LocalBroadcastManager.getInstance(mContext).sendBroadcast(connectionStatus);
                BluetoothConnectionStatus = false;
